...
const MarkdownExcerpt: React.FC<Props> = ({ content }) => {
  ...
  return (
    <div
      ref={ref}
      className="relative text-gray-400 mt-1 text-sm leading-6 max-h-[800px] overflow-hidden"
    >
      <div>
        <ReactMarkdown
          skipHtml
          ...
          components={{
            ...
            table: ({ node, ...props }) => (
              <div className="overflow-x-auto mt-2">
                <table className="w-full text-left border-collapse text-xs">{props.children}</table>
              </div>
            ),
            th: ({ node, ...props }) => (
              <th className="px-2 py-1 border-b border-white/10" {...props} />
            ),
            td: ({ node, ...props }) => (
              <td className="px-2 py-1 border-b border-white/10" {...props} />
            ),
